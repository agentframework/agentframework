...
describe('3.6. Get parameter attributes', () => {
  describe('# should able to', () => {
    ...
    it('get attribute of giving type', () => {
      ...
      expect(property.parameter(0).hasAttribute()).toBeFalse();
      expect(property.parameter(0).findAttributes((a) => a instanceof InjectAttribute)).toEqual([]);
      expect(property.parameter(0).getAttribute(InjectAttribute)).toBeUndefined();
      expect(property.parameter(0).getAttributes(InjectAttribute)).toEqual([]);

      expect(property.parameter(1).type).toBe(Object);
      expect(property.parameter(1).hasAttribute()).toBeTrue();
      expect(property.parameter(1).findAttributes((a) => a instanceof OptionalAttribute).length).toBe(1);
      expect(property.parameter(1).getAttribute(OptionalAttribute)).toBeInstanceOf(OptionalAttribute);
      expect(property.parameter(1).getAttributes(OptionalAttribute).length).toBe(1);
    });

    it('get all attributes of giving type', () => {
      const property = Reflector(UserController34).property('listAllUser');
      expect(property.parameter(0).getAttributes(OptionalAttribute).length).toBe(0);
      expect(property.parameter(1).getAttributes(OptionalAttribute).length).toBe(1);
    });

    it('get all attributes', () => {
      const property = Reflector(UserController34).property('listAllUser');
      expect(property.parameter(0).getAttributes().length).toBe(0);
      expect(property.parameter(1).getAttributes().length).toBe(1);
    });

    it('find attributes using inline filter function', () => {
      const property = Reflector(UserController34).property('listAllUser');
      expect(property.parameter(0).findAttributes((a) => !!a).length).toBe(0);
      expect(property.parameter(1).findAttributes((a) => !!a).length).toBe(1);
    });
    ...
    it('find attributes using external filter function with filter criteria', () => {
      ...
      const property = Reflector(UserController34).property('listAllUser');
      expect(property.parameter(0).findAttributes(InstanceOf, OptionalAttribute).length).toBe(0);
      expect(property.parameter(1).findAttributes(InstanceOf, OptionalAttribute).length).toBe(1);
    });

    it('get parameter attribute', () => {
      const attributes = Reflector(Controller35).parameter(0).getAttributes();
      expect(attributes.length).toBe(0);
    });

    it('find parameter attribute with interceptor', () => {
      const attributes = Reflector(Controller34).parameter(0).findAttributes(HasInterceptor);
    ...
    it('find parameter attribute with interceptor', () => {
      const attributes = Reflector(Controller34)
        .parameter(0)
        .findAttributes((a) => GetInterceptor(a) != null);
      expect(attributes.length).toBe(1);
    });

    it('find parameter attribute', () => {
      const attributes = Reflector(Controller35).parameter(0).findAttributes(HasInterceptor);
      expect(attributes.length).toBe(0);
    });
  ...
  describe('# should not able to', () => {
    ...
    it('get non-annotated parameter', () => {
      const results = Reflector(Base34).findAttributes(HasInterceptor);
